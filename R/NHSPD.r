...
.citations <- function() {
  s1 <- "Contains OS data (C) Crown copyright and database right [year]"
  s2 <- "Contains Royal Mail data (C) Royal Mail copyright and database right [year]"
  s3 <- "Contains National Statistics data (C) Crown copyright and database right [year]"
  return(sprintf("The NHSPD package process data that needs the following citations:\n  %s\n  %s\n  %s\n",s1,s2,s3))
}

# default source data files -----------------------------------------------

.default_gridall_DataFile <- "H:/DATASETS/HSCIC/gridall.csv"
...
# CodePoint stuff
.codelist.xl.filename <- "Codelist.xlsx" # 2015.4.0 version uses .xlsx; .xlsx needs $X0, not $X1
...
#'
#' @return compiled list of all the area codes
#' @export
#'
#' @examples
getCodePointAreaCodes <- function( codepoint.doc.path = paste(.default_CodePoint_Unzipped_Path, "Doc", sep = "/") ) {
  ...
  area_codes <- as.list(read_excel(codelist.xl.file, sheet = .codelist.xl.areacodes.sheetname, col_names = FALSE)[,1])$X0 # X1 for .xls, X0 for .xlsx
...
#' @param codepoint.doc.path see getCodePointAreaCodes()
#' @param CodePoint.compatible when remake.RDS - column names to be compatible with my previous CodePoint library
#'
#' @return data table
#' @export
#'
...
getNHSPD <- function( gridall.file = .default_gridall_DataFile,
  ...
  if (TRUE) { # remove terminated postcodes
    z <- subset(z, DOTERM == "")
